...
$passFromDb = pg_query($db_connection,"SELECT password FROM users WHERE email = '$email'");
$haspassres = pg_fetch_assoc($passFromDb);
$hashedpass = $haspassres['password'];

if (password_verify($password, $hashedpass)) 
{
$user = pg_query($db_connection,"SELECT count(*) AS rows FROM users WHERE email = '$email' and password = '$hashedpass'");
$userinfo = pg_query($db_connection,"SELECT * FROM users WHERE email = '$email' and password = '$hashedpass'");
